...
public class HourChecker {
	...
	private static boolean check(Worker[] arr, String toCheckValue)
    {
        ...
        for (Worker element : arr) {
            if (element.getNickname() == toCheckValue) {
                test++;
            }
        }
        if(test > 1) {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < workers; i++) {
				while(check(people,people[i].getNickname())==false) {
					people[i].setNickname();
				}
		}
		...
		for(int i = 0; i < workers; i++) {
			System.out.println((i+1)+"."+people[i].getNickname());
			System.out.println("   Working days:");
			people[i].printWorkingDays();
			System.out.println(people[i].getShift());
		}
		...
		if(h >= 9 && h <=17){
			System.out.println(seniors[0].getNickname().toUpperCase()+" is "+seniors[0].getStatus());
			for(int i = 0; i < workers; i++) {
				...
				if(!people[i].getStatus().equals("on a Free Day.")&&!people[i].getStatus().equals("Out of working hours.")) {
					System.out.println(people[i].getNickname());
				}
			}
		}else if(h >=12 && h <= 20){
			System.out.println(seniors[1].getNickname().toUpperCase()+" is "+seniors[1].getStatus());
			for(int i = 0; i < workers; i++) {
				people[i].checkWorkingHour(day,hour);
				if(!people[i].getStatus().equals("on a Free Day.")&&!people[i].getStatus().equals("Out of working hours.")) {
					System.out.println(people[i].getNickname());
			...
			System.out.println(seniors[2].getNickname().toUpperCase()+" is "+seniors[2].getStatus());
			for(int i = 0; i < workers; i++) {
				people[i].checkWorkingHour(day,hour);
				if(!people[i].getStatus().equals("on a Free Day.")&&!people[i].getStatus().equals("Out of working hours.")) {
					System.out.println(people[i].getNickname());
